...
if st.button("Go", on_click=callback) or st.session_state["btn_clicked"]:
    ...
    try:
        ...
        st.error(
            "Not possible to retrieve data for that ticker. Please check if its valid and try again."
        )
        sys.exit()
    ...
    try:
        ...
        with col4:
            ...
            st.metric(
                label="Price",
                value=company_data["Price"],
                delta=company_data["Price change"],
            )
            empty_lines(2)
        ...
        with col8:
            ...
            st.metric(
                label="Market Cap",
                value=millify(metrics_data["Market Cap"][0], precision=2),
                delta=delta(metrics_data, "Market Cap"),
            )
            st.write("")
            st.metric(
                label="D/E Ratio",
                value=round(metrics_data["D/E ratio"][0], 2),
                delta=delta(metrics_data, "D/E ratio"),
            )
            st.write("")
            st.metric(
                label="ROE",
                value=str(round(metrics_data["ROE"][0] * 100, 2)) + "%",
                delta=delta(metrics_data, "ROE"),
            )
        ...
        with col9:
            ...
            st.metric(
                ...
                value=millify(metrics_data["Working Capital"][0], precision=2),
                delta=delta(metrics_data, "Working Capital"),
            )
            st.write("")
            st.metric(
                label="P/E Ratio",
                value=round(metrics_data["P/E Ratio"][0], 2),
                delta=delta(metrics_data, "P/E Ratio"),
            )
            st.write("")
            ...
            if metrics_data["Dividend Yield"][0] == 0:
                ...
                st.metric(
                    ...
                    value=str(round(metrics_data["Dividend Yield"][0] * 100, 2)) + "%",
                    delta=delta(metrics_data, "Dividend Yield"),
                )
        with col10:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    ...
                    line=dict(width=0.8, color="rgba(255, 140, 0, 1)"),
                ),
            )
        )
    ...
    try:
        ...
        income_data.columns = income_data.columns.str.replace(
            r"[\/\(\)\-\+=]\s?", "", regex=True
        )
